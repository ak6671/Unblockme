...
export default function Login() {
  ...
  const requestOTP = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/auth/send-otp`, {
        ...
        body: JSON.stringify({ phone, mode, name: mode === 'signup' ? name : undefined })
      });
  ...
  const verifyOTP = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/auth/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, otp, name: mode === 'signup' ? name : undefined })
      });
      const data = await res.json();
  ...
  const handleLogout = () => {
    ...
    window.location.reload(); // Quick refresh to clear the logged in state
  };
          ...
          {isLoggedIn ? (
            ...
            >
              <div>
                ...
                {mockOtp && (
                  <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl p-4 mb-4 text-sm font-semibold flex items-center justify-between shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                    <div className="flex items-center gap-2">
                      <span className="text-base">🔑</span>
                      <span>Demo Mode OTP: <strong className="font-extrabold text-lg select-all bg-amber-100/80 px-2 py-0.5 rounded text-amber-900 font-mono tracking-wider">{mockOtp}</strong></span>
                    </div>
                    <button 
                      type="button" 
                      onClick={() => {
                      ...
                      className="text-xs font-black text-amber-700 hover:text-amber-950 bg-amber-100 hover:bg-amber-200 px-3 py-1.5 rounded-lg border border-amber-300 transition-all uppercase tracking-wider cursor-pointer active:scale-95"
                    >
                      Auto Fill
                    </button>
                  </div>
              ...
              <div className="text-center pt-2">
                <button 
                  ...
                  className="text-sm text-gray-400 hover:text-gray-800 font-bold transition-colors"
                >
